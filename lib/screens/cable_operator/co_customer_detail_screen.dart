...
class CoCustomerDetailScreen extends StatefulWidget {
  final String customerId;
  const CoCustomerDetailScreen({super.key, required this.customerId});

  @override
  State<CoCustomerDetailScreen> createState() => _CoCustomerDetailScreenState();
}

class _CoCustomerDetailScreenState extends State<CoCustomerDetailScreen>
    with SingleTickerProviderStateMixin {
  final _custRepo = CustomersRepository();
  final _compRepo = ComplaintsRepository();
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _customer = cust;
      _complaints =
          all.where((c) => c.customerId == widget.customerId).toList();
    } catch (e) {
      _error = e.toString();
    } finally {
          ...
          : _error != null
              ...
              : _customer == null
                  ...
                  : TabBarView(
                      controller: _tabs,
                      children: [
                        _CoInfoTab(customer: _customer!, pn: pn),
...
class _CoInfoTab extends StatelessWidget {
  final Customer customer;
  final PnColors pn;
  const _CoInfoTab({required this.customer, required this.pn});
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        // ONU card — prominent for cable operator
        if (customer.onuNumber != null)
          Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.router, color: primary, size: 28),
                ...
                GestureDetector(
                  onTap: () {
                    Clipboard.setData(
                        ClipboardData(text: customer.onuNumber!));
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text('ONU number copied'),
                          duration: Duration(seconds: 1)),
                    );
                  },
                  child: Icon(Icons.copy, size: 18, color: pn.textMuted),
                ),
              ],
            ),
          ),
        _Section(
          ...
          children: [
            ...
            _Row(
              label: 'IPTV',
              value: customer.iptv ? 'Enabled' : 'Disabled',
              pn: pn,
            ),
          ],
        ),
